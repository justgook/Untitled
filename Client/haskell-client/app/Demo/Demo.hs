...
import Control.Monad.RWS.Strict (ask, asks, evalRWST, get, liftIO, modify, put)

import qualified Graphics.Rendering.OpenGL as GL
import qualified Graphics.UI.GLFW as GLFW

import Demo.Gear (makeGear)

import Demo.Alias (Demo, Env(..), State(..))
...
run = do
  ...
  if stateDragging state
    then do
      ...
      let myrot = (x - sodx) / 2
          mxrot = (y - sody) / 2
      put $ state {stateXAngle = sodxa + mxrot, stateYAngle = sodya + myrot}
    else do
